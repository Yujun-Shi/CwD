...
class Appr(Inc_Learning_Appr):
    ...
    def __init__(self, model, device, nepochs=100, lr=0.05, decay_mile_stone=[80,120], clipgrad=10000,
                 momentum=0, wd=0, multi_softmax=False, wu_nepochs=0, wu_lr_factor=1, fix_bn=False, eval_on_train=False, ddp=False,
                 logger=None, exemplars_dataset=None, lamb=5000, alpha=0.5, fi_sampling_type='max_pred',
                 fi_num_samples=-1, save_fisher=False):
        super(Appr, self).__init__(model, device, nepochs, lr, decay_mile_stone, clipgrad, momentum, wd,
    ...
    def extra_parser(args):
        ...
        parser.add_argument('--save-fisher', action='store_true',
                            help='whether to save Fisher information')
    ...
    #             preds = Categorical(logits=outputs[-1]).sample()

    #         loss = torch.nn.functional.cross_entropy(outputs[-1], preds)
    ...
    def compute_fisher_matrix_diag(self, trn_loader, val_loader):
        # Store Fisher Information
        fisher = {n: torch.zeros(p.shape).to(self.device) for n, p in self.model.model.named_parameters()
                  if p.requires_grad}
        # Do forward and backward pass to compute the fisher information
        self.model.eval()
        with override_dataset_transform(trn_loader.dataset, val_loader.dataset.transform) as _ds:
            fisher_loader = DataLoader(_ds, batch_size=1, shuffle=False,
                num_workers=trn_loader.num_workers, pin_memory=trn_loader.pin_memory)
            n_samples = 0
            for images, targets in tqdm(fisher_loader):
    ...
    def post_train_process(self, t, trn_loader, val_loader):
        ...
        curr_fisher = self.compute_fisher_matrix_diag(trn_loader, val_loader)
        ...
        for n in self.fisher.keys():
            ...
            # directly adding more constraint
            self.fisher[n] = self.fisher[n] + curr_fisher[n]

        if self.save_fisher:
            torch.save(self.fisher, './fisher/lamb_{}_task_{}.pt'.format(self.lamb, t))
