...
class ExemplarsSelector:
    ...
    def __call__(self, model: LLL_Net, trn_loader: DataLoader, transform, ddp=False):
        clock0 = time.time()
        ...
        with override_dataset_transform(trn_loader.dataset, transform) as ds_for_selection:
            ...
            if ddp:
                # make sure all process using the same exemplar set
                selected_indices = torch.from_numpy(np.array(selected_indices)).cuda()
                torch.distributed.broadcast(selected_indices, src=0)
