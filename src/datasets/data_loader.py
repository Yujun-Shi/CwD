...
def get_loaders(datasets, num_tasks, nc_first_task, batch_size, num_workers, pin_memory, validation=.1, ddp=False):
    ...
    for idx_dataset, cur_dataset in enumerate(datasets, 0):
        ...
        trn_transform, tst_transform = get_transforms(resize=dc['resize'],
                                                      resize_test=dc['resize_test'],
                                                      pad=dc['pad'],
                                                      crop=dc['crop'],
                                                      cifar_crop=dc['cifar_crop'],
                                                      flip=dc['flip'],
        ...
        if ddp:
            for tt in range(num_tasks):
                trn_sampler = torch.utils.data.DistributedSampler(trn_dset[tt], shuffle=True)
                trn_load.append(data.DataLoader(trn_dset[tt], batch_size=batch_size, num_workers=num_workers,
                                                sampler=trn_sampler, pin_memory=pin_memory))
...
def get_datasets(dataset, path, num_tasks, nc_first_task, validation, trn_transform, tst_transform, class_order=None):
    ...
    if 'mnist' in dataset:
        ...
        all_data, taskcla, class_indices = basedat.get_data(path, num_tasks=num_tasks, nc_first_task=nc_first_task,
                                                            ...
                                                            class_order=class_order, trn_lst='train_100.txt', tst_lst='val_100.txt')
        # set dataset type
        ...
        all_data, taskcla, class_indices = basedat.get_data(path, num_tasks=num_tasks, nc_first_task=nc_first_task,
                                                            validation=validation, shuffle_classes=class_order is None,
                                                            class_order=class_order, trn_lst='train_1000.txt', tst_lst='val_1000.txt')
        # set dataset type
        Dataset = basedat.BaseDataset
...
def get_transforms(resize, resize_test, pad, crop, cifar_crop, flip, normalize, extend_channel):
    ...
    if resize_test is not None:
        tst_transform_list.append(transforms.Resize(resize_test))
    ...
    if cifar_crop is not None:
        trn_transform_list.append(transforms.RandomCrop(cifar_crop))
